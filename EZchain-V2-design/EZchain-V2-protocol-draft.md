...
3. 验证 `sender_addr == sidecar.sender_addr`
4. 验证 `chain_id`
5. 验证 `expiry_height`
6. 验证 `low-s`
7. 验证 `seq`
8. 验证大小限制
9. 验证 `BundleSidecar` 内所有 tx 的 sender 一致
...
因为 V2 不引入 `claim_set_hash`，链下验证仍需扫描 Bundle 内容，故大小限制非常重要。
...
1. 计算 `bundle_hash`
2. 根据链上最新已确认 seq 生成 `BundleEnvelope`
3. 签名
4. 广播给共识节点
...
由于 V2 不引入 `claim_set_hash`，recipient MUST 扫描目标 `ConfirmedBundleUnit` 的整个 `bundle_sidecar`，检查：
...
### 问题 4：不引入 `claim_set_hash` 的代价

不引入 `claim_set_hash` 是可以的，但会带来一个明确代价：

- recipient 必须扫描整个 `BundleSidecar`

解决：

- 强制设置 Bundle 大小上限
- 后续版本可再讨论是否引入 Value 集合承诺
...
2. 可选的 `claim_set_hash`
3. Receipt 归档服务节点
4. Witness 压缩与批量证明
5. 更智能的 Checkpoint 策略
