...
1. 功能概述
VPB 验证算法是 EZchain 区块链系统的核心验证机制, 用于接收方(Account Node)独立验证交易合法性。
其核心目标是通过 “价值(Value)- 证明(Proofs)- 区块索引(Block index list)” 三元组, 
结合区块链主链数据与检查点(Checkpoint)机制, 在不追溯全量交易历史的前提下, 
高效验证价值转移的合法性(无双重支付、所有权变更有效、交易信息未篡改等)。

2. 输入
VPB_pair - 待验证的VPB对
main_chain_info - 主链信息(包含默克尔树根、Bloom Filter、区块号等)
checkpoint - 验证的检查点(可选)

3. 输出
验证结果(True/False)及错误信息(如有)

4. 验证步骤
 - 检测输入的VPB_pair格式是否正确
 - 检测VPB_pair中的Value是否命中account的checkpoint
 - 若有命中, 则无需验证此VPB_pair在checkpoint标记的区块高度之前的历史
 - 根据checkpoint生成此Value待检验的“历史切片”。
 - 待检验的“历史切片”中的数据的正确性(即, 能够和主链的默克尔树根+布隆过滤器正确匹配, 且交易数据均合法合规(比如交易数值不能超过value本身的num之类的))
 - 验证Value的“历史切片”中是否存在双花行为
 - 其他合法性验证: 签名等
