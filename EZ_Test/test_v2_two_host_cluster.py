...
class V2TwoHostClusterTests(unittest.TestCase):
    ...
    def test_run_tx_batch_waits_for_receipt_after_pending_clears(self) -> None:
        topology = _build_topology(
            cluster_name="demo",
            chain_id=821,
            mac_consensus_count=2,
        ...
        def fake_state(path: Path):
            ...
            receipt_count = 0
            chain_cursor = {"height": 0, "block_hash_hex": "00"}
            if state_calls[node_id] >= 4:
                receipt_count = 1
                chain_cursor = {"height": 9, "block_hash_hex": "late-receipt"}
            return {
                "address": "0xsender",
                "pending_bundle_count": 0,
                "consensus_endpoint": "127.0.0.1:19500",
        ...
        self.assertEqual(result["failed"], 0)
        self.assertEqual(result["results"][0]["status"], "confirmed")
        self.assertEqual(result["results"][0]["receipt_height"], 9)
        self.assertEqual(result["results"][0]["receipt_block_hash"], "late-receipt")
