...
class EZV2ProtocolWalletWitnessTests(unittest.TestCase):
    ...
    def test_recursive_witness_validation_and_checkpoint_anchor(self) -> None:
        ...
        tx_g1 = OffChainTx(sender_addr=grace_addr, recipient_addr=alice_addr, value_list=(ValueRange(2700, 2749),), tx_local_index=0, tx_time=1)
        ...
        witness_g_to_a = WitnessV2(
            ...
            anchor=GenesisAnchor(genesis_block_hash=b"\x11" * 32, first_owner_addr=grace_addr, value_begin=2700, value_end=2999),
        )

        tx_a1 = OffChainTx(sender_addr=alice_addr, recipient_addr=carol_addr, value_list=(ValueRange(2700, 2749),), tx_local_index=0, tx_time=2)
        ...
        validator = V2TransferValidator(ValidationContext(genesis_allocations={grace_addr: (ValueRange(2700, 2999),)}))
        ...
        checkpoint_validator = V2TransferValidator(
            ValidationContext(trusted_checkpoints=(checkpoint,), genesis_allocations={grace_addr: (ValueRange(2700, 2999),)})
        )
        checkpoint_package = TransferPackage(target_tx=tx_a1, target_value=ValueRange(2700, 2749), witness_v2=witness_a_to_c)
    ...
    def test_wallet_acquisition_boundary_updates(self) -> None:
        ...
        tx_a1 = OffChainTx(sender_addr=alice_addr, recipient_addr=bob_addr, value_list=(ValueRange(0, 99),), tx_local_index=0, tx_time=1)
        ...
        tx_b1 = OffChainTx(sender_addr=bob_addr, recipient_addr=alice_addr, value_list=(ValueRange(2700, 2749),), tx_local_index=0, tx_time=1)
        sub_b1 = self._make_submission(bob_priv, bob_pub, 21, 1, 10, 1, 7, [tx_b1])
        chain.submit_bundle(sub_b1)
        _, receipts_b1 = chain.build_block(timestamp=11)
        ...
        prior_witness = WitnessV2(
            ...
            anchor=GenesisAnchor(genesis_block_hash=b"\x44" * 32, first_owner_addr=bob_addr, value_begin=2700, value_end=2999),
        )
        incoming_record = wallet.receive_transfer(
            TransferPackage(target_tx=tx_b1, target_value=ValueRange(2700, 2749), witness_v2=prior_witness),
            validator=V2TransferValidator(ValidationContext(genesis_allocations={bob_addr: (ValueRange(2700, 2999),)})),
        )

        tx_a2_0 = OffChainTx(sender_addr=alice_addr, recipient_addr=bob_addr, value_list=(ValueRange(2700, 2749),), tx_local_index=0, tx_time=2)
        tx_a2_1 = OffChainTx(sender_addr=alice_addr, recipient_addr=bob_addr, value_list=(ValueRange(300, 349),), tx_local_index=1, tx_time=2)
        ...
        updated = wallet.apply_sender_confirmed_unit(unit_a2, outgoing_values=(ValueRange(2700, 2749), ValueRange(300, 349)))
        ...
        self.assertEqual([unit.receipt.seq for unit in retained.witness_v2.confirmed_bundle_chain], [2, 1])
        self.assertEqual([unit.receipt.seq for unit in archived_received.witness_v2.confirmed_bundle_chain], [2])
