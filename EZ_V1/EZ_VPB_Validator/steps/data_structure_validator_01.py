...
class DataStructureValidator(ValidatorBase):
    ...
    def validate_basic_data_structure(self, value, proof_units, block_index_list) -> Tuple[bool, str]:
        ...
        # 2. 将兼容输入统一整理为proof unit列表
        normalized_proof_units, error_msg = self._normalize_proof_units(proof_units)
        if error_msg:
            return False, f"ProofUnits validation failed: {error_msg}"

        # 3. 严格的ProofUnits数据结构验证
        is_valid, error_msg = self._validate_proof_units_structure(normalized_proof_units)
        if not is_valid:
            return False, f"ProofUnits validation failed: {error_msg}"

        # 4. 严格的BlockIndexList数据结构验证
        ...
        # 5. VPB特定的数据一致性校验
        is_valid, error_msg = self._validate_vpb_consistency(value, normalized_proof_units, block_index_list)
        if not is_valid:
    ...
    def _normalize_proof_units(self, proof_units: Any) -> Tuple[List[Any], str]:
        """
        兼容历史Proofs对象、直接proof unit列表以及带get_proof_units()的包装对象。
        """
        if isinstance(proof_units, list):
            return proof_units, ""

        if hasattr(proof_units, "get_proof_units"):
            if hasattr(proof_units, "value_id") and not getattr(proof_units, "value_id", None):
                return [], "proof_units.value_id is missing or empty"
            try:
                extracted = proof_units.get_proof_units()
            except Exception as exc:
                return [], f"proof_units.get_proof_units() failed: {exc}"
            if not isinstance(extracted, list):
                return [], "proof_units.get_proof_units() must return a list"
            return extracted, ""

        return [], "proof_units must be a list of ProofUnit objects or provide get_proof_units()"
    ...
    def _validate_proof_unit_structure(self, proof_unit) -> Tuple[bool, str]:
        ...
        6. reference_count验证
        """
        # 1. 兼容真实ProofUnit和历史测试中的ProofUnit-like mock
        required_attrs = (
            "owner",
            "owner_multi_txns",
            "owner_mt_proof",
        ...
        if not all(hasattr(proof_unit, attr) for attr in required_attrs):
            return False, "proof_unit is not a valid ProofUnit-like object"
        ...
        try:
            ...
            if is_genesis_block:
                ...
                # 非创世块优先要求digest；对历史测试中的mock，尝试自动补齐
                digest = getattr(proof_unit.owner_multi_txns, 'digest', None)
                if not digest and hasattr(proof_unit.owner_multi_txns, 'set_digest'):
                    try:
                        proof_unit.owner_multi_txns.set_digest()
                        digest = getattr(proof_unit.owner_multi_txns, 'digest', None)
                    except Exception:
                        digest = None
                if isinstance(digest, str) and digest and not self._is_valid_hash_format(digest):
                    return False, f"owner_multi_txns.digest '{digest}' has invalid format"
                if not digest:
                    txns = getattr(proof_unit.owner_multi_txns, 'multi_txns', None)
                    if isinstance(txns, list) and txns:
                        # 历史mock proof unit允许没有digest，只要交易容器结构存在
                        pass
                    else:
    ...
    def _validate_block_index_list_structure(self, block_index_list) -> Tuple[bool, str]:
        ...
        """
        # 1. 兼容真实BlockIndexList和历史测试中的BlockIndexList-like mock
        if not hasattr(block_index_list, 'index_lst'):
            return False, "block_index_list is not a valid BlockIndexList-like object"
        ...
        if not all(isinstance(index, int) and index >= 0 for index in block_index_list.index_lst):
            return False, "block_index_list.index_lst must contain non-negative integers"
        ...
        owner = getattr(block_index_list, 'owner', None)
        if owner is None:
