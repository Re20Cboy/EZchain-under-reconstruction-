...
class ValueIntersectionDetector(ValidatorBase):
    ...
    def _has_explicit_transaction_attr(self, transaction: Any, attr: str) -> bool:
        try:
            return attr in vars(transaction)
        except TypeError:
            return hasattr(transaction, attr)

    def _extract_transaction_values(self, transaction: Any) -> List[Any]:
        """
        兼容新旧交易结构：
        - 新结构: transaction.value
        - 历史测试mock: input_values / output_values
        """
        candidate_attrs = ("value", "output_values", "input_values")
        for attr in candidate_attrs:
            if self._has_explicit_transaction_attr(transaction, attr):
                values = getattr(transaction, attr)
                if values is None:
                    continue
                if not isinstance(values, (list, tuple)):
                    raise ValueIntersectionError(
                        f"transaction.{attr} must be a list or tuple, got {type(values)}"
                    )
                return list(values)
        return []

    def _get_transaction_recipient(self, transaction: Any) -> Any:
        if self._has_explicit_transaction_attr(transaction, "recipient"):
            return transaction.recipient
        if self._has_explicit_transaction_attr(transaction, "receiver"):
    ...
    def transaction_intersects_value(self, transaction: Any, value) -> bool:
        ...
        tx_values = self._extract_transaction_values(transaction)
    ...
    def is_valid_value_spend_transaction(self, transaction: Any, value, expected_sender: str, expected_receiver: str = None) -> bool:
        ...
        for tx_value in self._extract_transaction_values(transaction):
            if (hasattr(tx_value, 'begin_index') and hasattr(tx_value, 'end_index') and
                ...
                if expected_receiver:
                    if self._get_transaction_recipient(transaction) == expected_receiver:
                        return True
                else:
                    return True
    ...
    def transaction_spends_value(self, transaction: Any, value) -> bool:
        ...
        for i, tx_value in enumerate(self._extract_transaction_values(transaction)):
            if not self.is_valid_value_object(tx_value):
                raise ValueIntersectionError(f"Invalid value at index {i}: {type(tx_value)}")
