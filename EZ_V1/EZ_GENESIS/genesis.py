...
class GenesisBlockCreator:
    ...
    def create_genesis_block(self, accounts: List["Account"],
                             custom_miner: Optional[str] = None) -> Tuple[Block, List[SubmitTxInfo]]:
        """
        Compatibility wrapper for legacy tests that expect:
        (genesis_block, [submit_tx_info, ...]).
        """
        genesis_block, submit_tx_info, _multi_txn, _merkle_tree = self.create_unified_genesis_block(
            accounts,
            custom_miner=custom_miner,
        )
        return genesis_block, [submit_tx_info]
    ...
    def _build_genesis_merkle_tree(self, transactions: List[object]) -> Tuple[MerkleTree, List[MerkleTreeProof]]:
        """
        Compatibility wrapper for older tests that pass a list of digest-like
        objects and expect a Merkle tree plus per-leaf proofs.
        """
        digests = []
        for txn in transactions:
            digest = getattr(txn, "digest", None)
            if digest is None:
                raise ValueError("transaction missing digest")
            digests.append(digest)

        merkle_tree = MerkleTree(digests)
        proofs = [MerkleTreeProof(path) for path in getattr(merkle_tree, "prf_list", [])]
        return merkle_tree, proofs
