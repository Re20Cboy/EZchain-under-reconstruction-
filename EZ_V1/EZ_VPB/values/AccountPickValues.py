...
class AccountPickValues:
    ...
    def __init__(self, account_address: str, existing_collection: AccountValueCollection):
        ...
        if existing_collection.account_address != account_address:
            raise ValueError(
                f"提供的AccountValueCollection账户地址({existing_collection.account_address})与指定账户地址({account_address})不匹配"
            )
    ...
    def pick_values_for_transaction(
        self,
        required_amount: int,
        sender: str,
        recipient: str,
    ...
    ) -> Tuple[List[Value], Optional[Value], Optional["Transaction"], Optional["Transaction"]]:
        """
        为交易选择Value。

        当前保留历史四元组接口：
        `(selected_values, change_value, change_tx, main_tx)`。
        ...
        Raises:
            ValueError: 如果余额不足或金额非法
        """
        # checkpoint 优先策略暂不改变旧接口行为；历史调用主要依赖找零协议。
        selected_values, change_value, change_transaction, main_transaction = (
            self.pick_values_for_transaction_with_change_legacy(
                required_amount, sender, recipient, nonce, time
            )
        )

        for value in selected_values:
            self._update_value_state(value, ValueState.PENDING)

        return selected_values, change_value, change_transaction, main_transaction

    def pick_values_for_transaction_with_change_legacy(
        self,
        required_amount: int,
        sender: str,
        recipient: str,
    ...
    def commit_transaction_values(self, selected_values: List[Value]) -> bool:
        """提交交易，将已选中的Value状态更新为ONCHAIN。"""
        for value in selected_values:
            self._update_value_state(value, ValueState.ONCHAIN)
        return True
    ...
    def _value_matches_checkpoint(
        self, value: Value, checkpoint: "CheckPointRecord", recipient: str
    ) -> bool:
        """
        检查value是否匹配检查点记录
    ...
    def _select_values_without_change(
        self, available_values: List[Value], target_amount: int
