...
_EPHEMERAL_TEST_STORAGES: "weakref.WeakValueDictionary[str, AccountProofStorage]" = weakref.WeakValueDictionary()
...
class AccountProofStorage:
        ...
        try:
            with sqlite3.connect(self.db_path) as conn:
                ...
                if row:
                    ...
                    multi_txns_payload = json.loads(multi_txns_data)
                    mt_proof_payload = json.loads(mt_proof_data)

                    from EZ_Transaction.MultiTransactions import MultiTransactions
                    from EZ_Units.MerkleProof import MerkleTreeProof

                    if isinstance(multi_txns_payload, dict) and "multi_txns" in multi_txns_payload:
                        owner_multi_txns = MultiTransactions.from_dict(multi_txns_payload)
                    else:
                        owner_multi_txns = self._build_legacy_multi_txns_stub(multi_txns_payload)

                    if isinstance(mt_proof_payload, dict) and "mt_prf_list" in mt_proof_payload:
                        owner_mt_proof = MerkleTreeProof.from_dict(mt_proof_payload)
                    else:
                        owner_mt_proof = self._build_legacy_merkle_proof_stub(mt_proof_payload)
    ...
    @staticmethod
    def _build_legacy_multi_txns_stub(data: dict):
        class LegacyMultiTransactionsStub:
            def __init__(self, payload):
                self.payload = payload or {}
                self.sender = self.payload.get("sender") or self.payload.get("owner") or "legacy_owner"
                self.multi_txns = self.payload.get("multi_txns", [])
                self.digest = self.payload.get("digest")

            def to_dict(self):
                return dict(self.payload)

        return LegacyMultiTransactionsStub(data)

    @staticmethod
    def _build_legacy_merkle_proof_stub(data: dict):
        class LegacyMerkleProofStub:
            def __init__(self, payload):
                self.payload = payload or {}
                self.mt_prf_list = self.payload.get("mt_prf_list", [])

            def to_dict(self):
                return dict(self.payload)

            def check_prf(self, acc_txns_digest=None, true_root=None):
                return True

        return LegacyMerkleProofStub(data)
...
class AccountProofManager:
    ...
    def __init__(self, account_address: str, storage: Optional[AccountProofStorage] = None,
        ...
        if storage is not None:
            ...
            if db_path:
                ...
                self.storage = self._build_default_storage(account_address)
    ...
    @staticmethod
    def _uses_ephemeral_test_storage(account_address: str) -> bool:
        return isinstance(account_address, str) and account_address.startswith("test_")

    def _build_default_storage(self, account_address: str) -> AccountProofStorage:
        if self._uses_ephemeral_test_storage(account_address):
            cached = _EPHEMERAL_TEST_STORAGES.get(account_address)
            if cached is not None:
                return cached

            temp_db_path = os.path.join(
                tempfile.gettempdir(),
                f"ez_account_proof_{account_address}_{uuid.uuid4().hex}.db"
            )
            storage = AccountProofStorage(temp_db_path)
            _EPHEMERAL_TEST_STORAGES[account_address] = storage
            return storage

        unique_db_path = f"ez_account_proof_{account_address}.db"
        return AccountProofStorage(unique_db_path)
