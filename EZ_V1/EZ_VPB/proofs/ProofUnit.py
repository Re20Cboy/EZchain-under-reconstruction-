...
class ProofUnit:  # value proof within a block
    ...
    def verify_proof_unit(self, merkle_root: str = None) -> tuple[bool, str]:
        ...
        """
        try:
            # 1. Verify that the proof unit owner is the canonical sender for this bundle.
            if self.owner_multi_txns.sender != self.owner:
                return False, (
                    f"MultiTransactions sender '{self.owner_multi_txns.sender}' "
                    f"does not match owner '{self.owner}'"
                )

            for i, txn in enumerate(self.owner_multi_txns.multi_txns):
                txn_sender = getattr(txn, 'sender', None)
                txn_payer = getattr(txn, 'payer', None)
                if txn_sender != self.owner and txn_payer != self.owner:
                    return False, (
                        f"Transaction[{i}] sender/payer does not match owner '{self.owner}'"
                    )
